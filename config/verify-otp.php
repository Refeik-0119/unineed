...
if (empty($_SESSION['forgot_user_id'])) {
    header('Location: /unineed/config/forgot-password.php');
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['verify_otp'])) {
    ...
    $query = "SELECT id FROM password_resets WHERE user_id = $user_id AND otp_hash = '$otp_hash' AND expires_at > NOW() LIMIT 1";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['reset_password']) && ($_SESSION['otp_verified'] ?? false)) {
    ...
    if (empty($new_password) || empty($confirm_password)) {
        ...
        if (mysqli_query($conn, $update_query)) {
            ...
            error_log("Password reset failed for user_id $user_id: " . mysqli_error($conn));
        }
    }
}
    ...
    <div class="reset-container">
        ...
        <?php if (!$show_password_form && !$success): ?>
            ...
            <form method="POST" action="" id="otpForm">
                <div class="mb-4">
                    ...
                    <input type="text" class="form-control" name="otp" id="otp" maxlength="6" placeholder="000000" required pattern="[0-9]{6}">
                    <small class="text-muted d-block mt-2">Enter the 6-digit code from your email</small>
                </div>
                <button type="submit" name="verify_otp" class="btn btn-primary btn-reset w-100">
                    <i class="bi bi-check-circle me-2"></i>Verify OTP
                </button>
            </form>
            
            <div class="text-center mt-3">
                <small class="text-muted">Didn't receive the OTP? <a href="/unineed/config/forgot-password.php" class="text-decoration-none" style="color: #61B087;">Request a new one</a></small>
            </div>
        <?php elseif ($show_password_form && !$success): ?>
            <!-- Password Reset Form -->
            <p class="text-muted text-center mb-4">Create a new password for your account.</p>
        ...
        <?php elseif ($success): ?>
            <div class="text-center">
                <a href="/unineed/config/index.php" class="btn btn-primary btn-reset w-100">
